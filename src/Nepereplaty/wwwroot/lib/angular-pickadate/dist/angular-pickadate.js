  ...
  angular.module('pickadate', [])
    ...
    .factory('pickadateDateHelper', ['$locale', 'dateFilter', function($locale, dateFilter) {
      ...
      return function(format, options) {
          var minDate, maxDate, disabledDates, currentDate, weekStartsOn, noExtraRows,years,lastMount,nextMount;
        ...
        return {
          ...
          setRestrictions: function (restrictions) {
              ...
              maxDate       = this.parseDate(restrictions.maxDate) || new Date(2017,1);
            
              currentDate = restrictions.currentDate;
              disabledDates = restrictions.disabledDates || [];
              nextMount = lastMount = currentDate;

              //   nextMount.setMonth(currentDate.getMonth() + 1) 
              //   lastMount.setMonth(currentDate.getMonth() - 1)
              console.log(minDate.getFullYear() , currentDate.getFullYear());
              if (minDate.getFullYear() == currentDate.getFullYear()) {
            ...
            restrictions.years = years;
            console.log(currentDate);
          },

          allowPrevMonth: function () {
            console.log(minDate);
            return currentDate.getMonth()-1 > 11;
          },
        ...
        '<div class="pickadate" ng-show="displayPicker" ng-style="modalStyles">' +
          '<div class="pickadate-header">' +
            '<div class="pickadate-controls">' +
              '<a class = "empty-pickadate-prev-year" ng-show="!allowPrevMonth"></a><a href="" class="pickadate-prev-year" ng-click="changeMonth(-1)" ng-show="allowPrevMonth">' +
                '{{lastMount | date:"MMM"}}' +
              '</a>' +
...
//                '<select></select>'+
'</h3><div dropdown-select="years" dropdown-onchange="setYear(selectCurrentYear.value)" dropdown-model="selectCurrentYear" dropdown-item-label="text"></div>' +
 //'<select  name="repeatSelect" id="repeatSelect" ng-options="option.name for option in years track by option.id"  ng-model="selectCurrentYear" ng-change="setYear(selectCurrentYear.id)" class="form-control" id="source"></select></h3>' +
//              '<input type="number" name="quantity" min="1926" max="2050" ng-model="currentYear" ng-change="setYear(currentYear)"></h3>' +
              
              '<a href="" class="pickadate-next-year" ng-click="changeMonth(1)" ng-show="allowNextMonth">' +
                '{{nextMount | date:"MMM"}}' +
              '</a><a class = "empty-pickadate-next-year" ng-show="!allowNextMonth"></a>' +
            '</div>'+
          '</div>' +
      ...
      return {
        ...
        link: function(scope, element, attrs, ngModel)  {
          ...
          var $render = ngModel.$render = function(options) {
            ...
            scope.selectCurrentYear = { value: scope.currentYear, text: scope.currentYear }
          ...
          scope.changeYear = function (offset) {
              if (!offset) return;
              scope.currentDate.setYear(scope.currentDate.getFullYear() + offset);
              render();
          };
		 
		  scope.setYear = function(set){
		  if (!set) return;
              scope.currentDate.setYear(set);
              render();
		  };
          ...
          function render() {
            ...
            scope.nextMount = new Date(scope.currentDate.getFullYear(),scope.currentDate.getMonth()+1)
            scope.lastMount = new Date(scope.currentDate.getFullYear(), scope.currentDate.getMonth() - 1)
            scope.allowPrevMonth = !(scope.lastMount.getMonth() == 11);
            scope.allowNextMonth = !(scope.nextMount.getMonth() == 0);
